...
# key 38F47D3E410C47B1: public key "Vladimir Sitnikov <vladimirsitnikov@apache.org>" imported
RUN wget -O /liquibase/lib/postgresql.jar https://repo1.maven.org/maven2/org/postgresql/postgresql/42.2.12/postgresql-42.2.12.jar \
    ...
    && rm -r "$GNUPGHOME" /liquibase/lib/postgresql.jar.asc

# MSSQL JDBC Driver
# Unable to locate Microsoft Keyserver, relying on SHA256SUM
ARG MSSQL_SHA256=36971479c6b3cd2569c52d911f78922e21c363b874b49392b21c37dfceaa2574
RUN wget -O /liquibase/lib/mssql.jar https://repo1.maven.org/maven2/com/microsoft/sqlserver/mssql-jdbc/8.2.2.jre13/mssql-jdbc-8.2.2.jre13.jar \
	&& echo "$MSSQL_SHA256  /liquibase/lib/mssql.jar" | sha256sum -c -

# key 01B0B092A6925976: public key "jdbcdrop@microsoft.com" imported
#RUN wget -O /liquibase/lib/mssql.jar https://repo1.maven.org/maven2/com/microsoft/sqlserver/mssql-jdbc/8.2.2.jre13/mssql-jdbc-8.2.2.jre13.jar \
...
#    && rm -r "$GNUPGHOME" /liquibase/lib/mssql.jar.asc

# MySQL JDBC Driver
# No GPG, using SHA256SUM
...
RUN wget -O /liquibase/lib/mariadb.jar https://repo1.maven.org/maven2/org/mariadb/jdbc/mariadb-java-client/2.6.0/mariadb-java-client-2.6.0.jar
    ...
    && rm -r "$GNUPGHOME" /liquibase/lib/mariadb.jar.asc
...
RUN wget -O /liquibase/lib/h2.jar https://repo1.maven.org/maven2/com/h2database/h2/1.4.200/h2-1.4.200.jar \
    ...
    && rm -r "$GNUPGHOME" /liquibase/lib/h2.jar.asc
...
RUN wget -O wget -O /liquibase/lib/db2.jar https://repo1.maven.org/maven2/com/ibm/db2/jcc/11.1.4.4/jcc-11.1.4.4.jar \
    ...
    && rm -r "$GNUPGHOME" /liquibase/lib/db2.jar.asc

# key 38F47D3E410C47B1: public key "Vladimir Sitnikov <vladimirsitnikov@apache.org>" imported
RUN wget -O wget -O /liquibase/lib/snowflake.jar https://repo1.maven.org/maven2/net/snowflake/snowflake-jdbc/3.12.3/snowflake-jdbc-3.12.3.jar \
    ...
    && rm -r "$GNUPGHOME" /liquibase/lib/snowflake.jar.asc
...
RUN wget -O /liquibase/lib/sybase.jar https://repo1.maven.org/maven2/net/sf/squirrel-sql/plugins/sybase/3.5.0/sybase-3.5.0.jar \
	&& wget -O wget -O /liquibase/lib/sybase.jar.asc https://repo1.maven.org/maven2/net/sf/squirrel-sql/plugins/sybase/3.5.0/sybase-3.5.0.jar.asc \
    && export GNUPGHOME="$(mktemp -d)" \
    && gpg --batch --keyserver ha.pool.sks-keyservers.net --recv-keys 38F47D3E410C47B1 \
    && gpg --batch --verify -fSLo /liquibase/lib/sybase.jar.asc /liquibase/lib/sybase.jar \
    && rm -r "$GNUPGHOME" /liquibase/lib/sybase.jar.asc
...
RUN wget -O /liquibase/lib/firebird.jar https://repo1.maven.org/maven2/net/sf/squirrel-sql/plugins/firebird/3.5.0/firebird-3.5.0.jar \
    ...
    && rm -r "$GNUPGHOME" /liquibase/lib/firebird.jar.asc

# key 38F47D3E410C47B1: public key "Vladimir Sitnikov <vladimirsitnikov@apache.org>" imported
RUN wget -O /liquibase/lib/sqlite.jar https://repo1.maven.org/maven2/org/xerial/sqlite-jdbc/3.31.1/sqlite-jdbc-3.31.1.jar \
	&& wget wget -O /liquibase/lib/sqlite.jar.asc https://repo1.maven.org/maven2/org/xerial/sqlite-jdbc/3.31.1/sqlite-jdbc-3.31.1.jar.asc \
    && export GNUPGHOME="$(mktemp -d)" \
    && gpg --batch --keyserver ha.pool.sks-keyservers.net --recv-keys 38F47D3E410C47B1 \
    && gpg --batch --verify -fSLo /liquibase/lib/sqllite.jar.asc /liquibase/lib/sqllite.jar \
    && rm -r "$GNUPGHOME" /liquibase/lib/sqllite.jar.asc
